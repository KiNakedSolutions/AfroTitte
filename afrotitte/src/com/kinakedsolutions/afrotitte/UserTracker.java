...
/**
 * AllInOne awt component that does user tracking. For now we will not mess with its ugly nature and simply use it as an 
 * entry point to our fun stuff.
 * 
 * TODO: replace System.out with log4j
 * 
 * @author some openni guy
 */
public class UserTracker extends Component
{
    ...
    // just some dirty testing, to be removed
    Vector3f xPointOld = null;
    Vector3f xPoint = null;
    
    //private final String SAMPLE_XML_FILE = "data/SamplesConfig.xml";
    private final String SAMPLE_XML_FILE = "data/OniConfig.xml";
    
    /**
     * Handles new Users, initiates their calibration if necessary.
     * @author some openni guy
     *
     */
	...
	/**
	 * Removes the joins of lost users.
	 * @author some openni guy
	 *
	 */
	...
	/**
	 * Checks if the calibration worked. If it did, it starts the tracking, if not it retries.
	 * @author some openni guy
	 *
	 */
	...
	/**
	 * Recalibrates after successful pose detection (?)
	 * @author some openni guy
	 *
	 */
    ...
    {
    	getJoints(user);
    	HashMap<SkeletonJoint, SkeletonJointPosition> dict = joints.get(Integer.valueOf(user));
    	...
    	Color debugColor = new Color(242,20,180);
    	g.setColor(debugColor);
    	
    	Point3D leftHipPos = dict.get(SkeletonJoint.LEFT_HIP).getPosition();
    	Point3D rightHipPos = dict.get(SkeletonJoint.RIGHT_HIP).getPosition();
    	
    	Point3D xPoint3D = new Point3D(
    			rightHipPos.getX() + (leftHipPos.getX() - rightHipPos.getX())/2,
    			rightHipPos.getY() + (leftHipPos.getY() - rightHipPos.getY())/2,
    			rightHipPos.getZ() + (leftHipPos.getZ() - rightHipPos.getZ())/2
    		);
    	float xSize = 55 - xPoint3D.getZ()/75;
    	int halfXSize = Math.round(xSize/2);
    	int xSizeInt = Math.round(xSize);
    	g.fillOval((int)xPoint3D.getX()-halfXSize, (int)xPoint3D.getY()-halfXSize, xSizeInt, xSizeInt);
    	g.drawString(String.valueOf(xPoint3D.getX()), 0, 10);
    	g.drawString(String.valueOf(xPoint3D.getY()), 0, 20);
    	g.drawString(String.valueOf(xPoint3D.getZ()), 0, 30);
    	g.drawString(String.valueOf(xSize), 0, 40);
    	
    	xPointOld = xPoint;
    	xPoint = new Vector3f((int)(xPoint3D.getX()/10), (int) (xPoint3D.getY()/10), (int) (xPoint3D.getZ()/10));
    	
    	
    	Vector3f xDiff = new Vector3f();
    	
    	if(xPointOld != null) {
    		xDiff.sub(xPoint, xPointOld);
    		
    	}
    	g.drawString(xDiff.z > 0 ? "OUT" : "IN", 0, 50);
    	g.drawString(String.valueOf(xDiff.lengthSquared()), 0, 60);
    	g.drawRect(0, 400,(int) (xDiff.lengthSquared()*10) , 10);
